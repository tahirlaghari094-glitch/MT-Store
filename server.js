...
// Increase payload limits for multiple Base64 assets upload
...
// Product Schema (Updated to support Multiple Images)
const ProductSchema = new mongoose.Schema({
    ...
    imageUrls: [String],       // 🖼️ Stores multiple images uploaded by the seller
    videoUrl: String,
...
const OrderSchema = new mongoose.Schema({
    ...
    buyerPhone: String,
    buyerAddress: String,
    status: { type: String, default: 'Processing' }, // Processing, Cancelled, Completed
    createdAt: { type: String, default: () => new Date().toISOString() }
});
...
app.get('/api/orders/user/:email', async (req, res) => {
    try {
        ...
        for (let order of userOrders) {
            ...
            updatedOrders.push({
                ...order._doc,
                productImage: prod ? (prod.imageUrl || (prod.imageUrls && prod.imageUrls[0]) || '') : '',
                productImages: prod ? prod.imageUrls : []
            });
        }
...
// Updated Product Submission API to accept Multiple Images array
app.post('/api/products', async (req, res) => {
    const { title, price, description, category, imageBase64, imageBase64Array, videoBase64, paymentDetails, address, contactNumber, sellerEmail, transactionId } = req.body;
    if (!title || !price || (!imageBase64 && (!imageBase64Array || imageBase64Array.length === 0)) || !paymentDetails || !transactionId) {
    ...
    try {
        ...
        // Handle image compilation nicely (single string fallback or array input)
        const finalImagesArray = imageBase64Array || (imageBase64 ? [imageBase64] : []);
        const primaryImage = finalImagesArray[0] || '';
...
// ==========================================
// ❌ NEW ORDER CANCELLATION PIPELINE
// ==========================================
app.post('/api/orders/cancel/:id', async (req, res) => {
    try {
        ...
        if (!order) return res.status(404).json({ error: "Order not found inside system database." });
        if (order.status !== 'Processing') return res.status(400).json({ error: "Only processing orders can be cancelled." });

        // Update local database status record
        order.status = 'Cancelled';
        await order.save();

        // Trace seller pipeline
        const linkedProduct = await Product.findOne({ id: order.productId });
        const sellerTargetEmail = linkedProduct ? linkedProduct.sellerEmail : null;

        const cancellationEmailHtml = `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; border: 1px solid #ef4444; border-radius: 16px; padding: 24px; max-width: 600px; background-color: #0b0f19; color: #f3f4f6;">
                <h2 style="color: #ef4444; border-bottom: 2px solid #ef4444; padding-bottom: 12px; margin-top: 0;">🚨 Alert: Order Cancelled By Buyer</h2>
                <p><strong>Order ID:</strong> <span style="color: #facc15; font-weight: bold;">${order.id}</span></p>
                <h3 style="color: #38bdf8; margin-bottom: 8px;">🛒 Cancelled Product Details</h3>
                <p style="margin: 4px 0;"><strong>Item Title:</strong> ${order.title}</p>
                <p style="margin: 4px 0;"><strong>Quantity:</strong> ${order.quantity}</p>
                <p style="margin: 4px 0;"><strong>Total Value:</strong> PKR ${order.price * order.quantity}</p>
                
                <div style="background-color: #1e1b4b; border: 1px solid #312e81; border-radius: 12px; padding: 16px; margin-top: 20px;">
                    <h3 style="color: #f87171; margin-top: 0; margin-bottom: 8px;">👤 Buyer Info</h3>
                    <p style="margin: 4px 0;"><strong>Name:</strong> ${order.buyerName}</p>
                    <p style="margin: 4px 0;"><strong>Email:</strong> ${order.buyerEmail}</p>
                </div>
            </div>
        `;

        // Send alert emails instantly
        await sendHtmlEmail(ADMIN_EMAIL, `🚨 Cancellation Alert: Order ${order.id}`, cancellationEmailHtml);
        if (sellerTargetEmail) {
            await sendHtmlEmail(sellerTargetEmail.toLowerCase(), `🚨 Customer Cancelled Order: ${order.id}`, cancellationEmailHtml);
        }

        res.json({ success: true, message: "Order cancelled and notifications sent successfully." });
    } catch (e) {
        console.error(e);
        res.status(500).json({ error: "Cancellation transmission pipeline failed." });
    }
});
